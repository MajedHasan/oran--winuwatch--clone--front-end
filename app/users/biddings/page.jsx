  ...
  useEffect(() => {
    async function fetchBids() {
      ...
      try {
        ...
        const res = await api.get("/bids/me", { headers });

        const now = new Date();

        const formattedBids = res.data.map((b) => {
          const drawDate = b.competition?.drawDate
            ? new Date(b.competition.drawDate)
            : null;

          let status = "Active";
          if (b.result === "won") status = "Won";
          else if (b.result === "lost") status = "Lost";
          else if (b.paymentStatus && b.paymentStatus !== "paid")
            status = b.paymentStatus;

          // If competition ended, mark as lost if not won
          if (drawDate && drawDate < now && status === "Active")
            status = "Lost";

          return {
            id: b._id,
            lottery: b.competition?.title || "Unknown Competition",
            bidAmount: Number(b.totalPrice || 0),
            tickets: b.tickets?.length || b.quantity || 0,
            status,
            endDate: drawDate ? drawDate.toLocaleDateString() : "N/A",
          };
        });
        ...
        {loading
          ? Array(4)
              ...
              .map((_, idx) => (
                <div
                  key={idx}
                  className="bg-[#1f1f1f] p-6 rounded-3xl animate-pulse h-48"
                />
              ))
          : filteredBids.map((bid) => (
              ...
              >
                <div>
                  ...
                  <p className="text-gray-300 mb-1">
                    Tickets:{" "}
                    <span className="font-semibold">{bid.tickets}</span>
                  </p>
          ...
          {loading
            ? Array(3)
                ...
                .map((_, idx) => (
                  <div
                    key={idx}
                    className="mb-6 ml-6 relative animate-pulse h-24 bg-[#1f1f1f] rounded-2xl"
                  />
                ))
            : bids
